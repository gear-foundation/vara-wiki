...
> **Note:**  
> This hook returns the account's total balance as a single value, not its individual components (such as free, reserved, or misc frozen balances). On Substrate-based chains, the balance type is a composite structure; to access specific fields like free balance, use the [useDeriveBalancesAll](/docs/api/tooling/react-hooks#usederivebalancesall).
...
Or in combination with [usePrepareProgramTransaction](/docs/api/tooling/react-hooks#useprepareprogramtransaction):
