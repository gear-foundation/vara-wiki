...
<Callout type="note" title="If the Relayer is unavailable, any bridging message remains stored on-chain and can be relayed manually at any time.">
No message is lost; manual relay simply finalizes the process and completes delivery.
</Callout>

<Callout type="warning" title="Always verify all contract addresses before bridging.">
...
<Callout type="note" title="After submitting the proof, the finalization (rewarding) process is not instant.">
...
<Callout type="note" title="Finalization and minting on Ethereum are **not instant**.">
