---
custom_title: Interaction Between Programs and Asynchronous Programming
...
<Callout type="warn" title="Async Macro Restrictions">
    While `async init` and `async main` can be used together, there are strict limitations:

    * You **cannot** define `init` or `handle_reply` functions if these macros are used.
    * Attempting to mix standard entry points with async macros will cause a compilation error.
</Callout>
