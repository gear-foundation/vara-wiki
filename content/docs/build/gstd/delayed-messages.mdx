...
Gear Protocol solves this issue by introducing delayed messaging functionality. Programs in Vara Network can execute themselves an **unlimited** number of blocks, as long as enough gas for execution is kept available. The [gas reservation](/docs/build/gstd/gas-reservation) option ensures this. As a result, the need for including centralized components in dApps is eliminated, allowing them to function **totally on-chain**.
