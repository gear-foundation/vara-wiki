...
<Callout type="info" title="Enabling Debug Macro">
    The `debug!` macro is only available when the **debug** feature is enabled in your `Cargo.toml`.

    Add it to your `gstd` dependencies:
