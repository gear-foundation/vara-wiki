---
title: Migration
custom_title: Migration of `gstd` programs
---

[`Sails`](/docs/developing/build/sails) is a library for writing applications utilizing the Gear Protocol with simplicity and clarity. This powerful tool represents the next step in the evolution of the Gear Protocol.

Initially, Gear applications typically employ the less convenient [`gstd`](/docs/developing/build/gstd) library for managing low-level operations and essential functionality. The `Sails` library builds programs on top of `gstd`, handling most low-level interactions under the hood. As projects grow, adopting the `Sails` framework allows developers to shift towards a more modular and service-oriented architecture. Existing dApps written with `gstd` can be effortlessly migrated to `Sails`.
...
- In **`Sails`**, this approach is restructured to leverage the framework’s service-oriented architecture. While the actions are still processed through a single entry point (`handle` function), logically they are dispatched to service methods based on string identifiers rather than enum types. Each action is linked to a specific service method, which improves modularity.
    ...
    ```
<Callout type="note" title="Sails vs gstd: Action Routing">
    Unlike the `gstd` approach of matching enum variants, the **Sails** framework uses string-based service identifiers (e.g., `do_a` or `do_b`).

    This mapping to service methods allows for more flexible and dynamic action routing within your programs.
</Callout>
...
(The `#[export]` attribute marks the method as part of the service's public ABI — without it the method is treated as a private helper.)
