...
To send messages, create programs, and subscribe to events using `Sails`, you need to connect to the chain using `@gear-js/api` and set the `GearApi` instance using the `setApi` method. For further details, refer to the [Gear-JS](/docs/developing/api) API section.
...
```javascript
import { getServiceNamePrefix, getFnNamePrefix } from 'vara-network-wiki/content/docs/developing/sails-js/index.mdx';
