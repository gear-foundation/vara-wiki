...
The [`@gear-js/react-hooks`](/docs/developing/api/tooling/react-hooks) library provides React hook abstractions over `sails-js` and its [generated program library](/docs/developing/sails-js/client-generation). This significantly simplifies the development of front-end applications built with Sails.
...
The `useSails` hook returns a `Sails` instance as described in the [Library Overview](/docs/developing/sails-js/overview#the-sails-class) section, based on the provided `programId` and IDL description of the deployed application.
...
The `useProgram` hook returns a `Program` instance as described in the [Client Generation](/docs/developing/sails-js/client-generation#methods-of-the-program-class) section using a generated library (`lib.ts`) and the `id` of a deployed application.
...
The `useSendProgramTransaction` hook is used to call a `function` (also referred to as a [command](/docs/developing/build/sails/coreconcepts#service) in Sails) of a Sails `service` from the application deployed at `id`. It returns a mutation to sign and send a transaction for this purpose.

> **Note:** The returned mutation callback is essentially a wrapper over the [`TransactionBuilder`](/docs/developing/sails-js/transaction-builder) in `sails-js` and its `signAndSend` method, which will be called upon mutation execution. The `useSendProgramTransaction` hook abstracts this process by internally specifying an account and a [`@gear-js/api`](/docs/developing/api) instance, making it a shortcut for this procedure.
...
Using the hook in combination with [useSendProgramTransaction](/docs/developing/sails-js/react-hooks#usesendprogramtransaction):
