...
The source code, developed using the [Sails](../../build/sails) framework, is available on [GitHub](https://github.com/gear-foundation/dapps/tree/master/contracts/tic-tac-toe). The exchange uses [Vara fungible tokens (VFT)](/docs/examples/Standards/vft).
...
- **dns_info**: Optional information for domain name system ([dDNS](../Infra/dein)) integration.
