...
<Callout type="info" title="Nightly builds are available if the latest features are needed: [nightly builds](https://get.gear.rs/#gear-nightly).">
...
<Callout type="info" title="To use the latest or experimental Gear Protocol's functions, compile the node from the `master` branch. It is the default branch after cloning, but switch back to the `master` branch using:">
...
<Callout type="note" title="The final build can be found at `gear/target/release/gear`.">
...
<Callout type="info" title="Regardless of whether the pre-built binary was downloaded or built manually, navigate to the directory where the node is installed and run it without special arguments to connect to the testnet:">
