...
The [`@gear-js/react-hooks`](/docs/api/tooling/react-hooks) library provides React hook abstractions over `sails-js` and its [generated program library](client-generation). This significantly simplifies the development of front-end applications built with Sails.
...
The `useSails` hook returns a `Sails` instance as described in the [Library Overview](overview#the-sails-class) section, based on the provided `programId` and IDL description of the deployed application.
...
The `useProgram` hook returns a `Program` instance as described in the [Client Generation](client-generation#methods-of-the-program-class) section using a generated library (`lib.ts`) and the `id` of a deployed application.
...
> **Note:** The returned mutation callback is essentially a wrapper over the [`TransactionBuilder`](transaction-builder) in `sails-js` and its `signAndSend` method, which will be called upon mutation execution. The `useSendProgramTransaction` hook abstracts this process by internally specifying an account and a [`@gear-js/api`](/docs/api) instance, making it a shortcut for this procedure.
