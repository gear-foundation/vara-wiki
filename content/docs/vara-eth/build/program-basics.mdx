...
```toml
[dependencies]
sails-rs = { version = "1.0.0", features = ["ethexe"] }
```
...
Sails 1.0.0 re-exports the SCALE codec and scale-info traits through the `#[sails_type]` attribute, so user code defining custom types no longer needs direct dependencies on `parity-scale-codec` or `scale-info`.
...
If your program uses custom structs or enums in messages or state,
annotate them with `#[sails_type]`. The attribute re-exports the SCALE
codec and scale-info traits with the correct paths — no manual `Encode`
/ `Decode` / `TypeInfo` derives or `#[codec(crate=...)]` boilerplate
needed.
