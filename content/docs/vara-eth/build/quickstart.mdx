...
<Callout type="note" title="Network Availability">
  Public Vara.eth RPC endpoints are currently offline between campaign phases. To experiment today, run a [full local stack](/docs/vara-eth/setup/local-node) and substitute your local RPC URL and Router address in the commands below.
</Callout>
