...
<Callout type="tip" title="Prefer to learn hands-on?">
  Try the **[Interactive Tutorial](https://eth.vara.network/tutorials/initialize/initialize)** — walk through the full on-chain flow (program creation, funding, message passing, get a reply) step by step, entirely in the browser. Zero local setup required.
</Callout>
