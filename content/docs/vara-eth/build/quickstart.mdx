...
<Callout type="note" title="Endpoint Availability">
  The `gear-tech.io` RPC endpoints below were operated by Gear for the launch campaigns and are no longer served. Substitute any Ethereum RPC provider for the chain you target — the Router addresses are unchanged and still deployed — or run a [full local stack](/docs/vara-eth/setup/local-node) and use your local RPC URL and Router address.
</Callout>
