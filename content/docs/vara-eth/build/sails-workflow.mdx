...
3. **[Gear IDEA](https://idea.gear-tech.io)** — to display methods in the explorer UI
...
```bash
npx sails-js generate --idl ./my_program.idl --out ./src/lib
```
...
```bash
sails-cli abi ./my_program.idl --out ./MyProgram.abi.json
```
...
4. Deploy using `ethexe` CLI or [Gear IDEA](https://idea.gear-tech.io)
