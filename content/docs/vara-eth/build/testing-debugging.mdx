...
After uploading, check that your code was validated:

```bash
ethexe query code-status --code-id 0x...
```

Expected output: `Validated`. If `Pending`, wait for the next executor batch. If validation failed, check your WASM module.

### Debug Message Failures

If your program doesn't respond to messages:

1. **Check Executable Balance** — Most common issue. Verify via `ethexe query balance --program 0x...`
