...
ethexe --cfg none tx \
  ...
  --sender "$SENDER" \
  upload target/wasm32-gear/release/my_program.opt.wasm -w
```
...
const tx = await client.uploadCode(wasmBlob, codeId);
await tx.wait();
```
...
3. `codeId` (keccak256 hash of WASM) is marked as pending

<Callout type="note" title="Deterministic Code ID">
  The `codeId` is the `keccak256` hash of your WASM binary. Same code always produces the same `codeId`, regardless of
  who uploads it.
</Callout>
...
```bash
ethexe query code-status --code-id 0x...
```

**Expected output:** `Validated`  
**If `Pending`:** Wait for the next executor batch  
**If failed:** Check validation errors in executor logs
...
- 256-bit `keccak256` hash of the WASM binary
