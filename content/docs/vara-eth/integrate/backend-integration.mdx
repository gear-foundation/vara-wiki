...
Direct connection to Vara.eth executors for real-time data and pre-confirmations:

```typescript
import { WebSocket } from "ws";

const ws = new WebSocket("wss://hoodi-reth-rpc.gear-tech.io/ws");

ws.on("open", () => {
  ws.send(
    JSON.stringify({
      jsonrpc: "2.0",
      method: "programStorage_stateQuery",
      params: [programId, "readMethod", encodedArgs],
      id: 1,
    })
  );
});
