...
<Callout type="note" title="Endpoint Availability">
  The `gear-tech.io` and `vara.network` URLs in the examples below were operated by Gear for the launch campaigns and are no longer served. Swap the Ethereum RPC for any Hoodi or Mainnet provider, and the Vara.eth RPC for [your own node](/docs/vara-eth/setup/local-node) — see [Network Endpoints](/docs/vara-eth/reference/network-endpoints). The code itself is unchanged.
</Callout>
...
<Callout type="note" title="Frontend Scope">
  Frontend usually owns interaction and reads. Upload/create/top-up are usually operational flows (backend scripts, CI, or
  manual CLI) and should not be hardcoded into browser UI.
</Callout>
...
```bash
cargo sails client-js ./path/to/your_program.idl ./src/generated/lib.ts
```
...
```bash
cargo sails client-js \
  ./target/wasm32-gear/release/my_program.idl \
  ./src/generated/lib.ts
```
