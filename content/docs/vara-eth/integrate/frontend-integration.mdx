...
| [@gear-js/api](/docs/vara-network/api) | Lower-level connection to Vara.eth RPC for subscriptions | `npm install @gear-js/api` |
...
export const config = {
  ...
  varaEthWs: "wss://testnet-ethexe.gear-tech.io/ws",
...
async function handleSwap(tokenA: string, tokenB: string, amount: bigint) {
  ...
  // 3. sails-js automatically fetches pre-confirmed result via RPC
  const result = tx.preConfirmedResult;

  // 4. Update UI immediately with pre-confirmed data
  setStatus("pre-confirmed");
  setBalance(result.newBalance);

  // 5. Optionally wait for L1 finality
