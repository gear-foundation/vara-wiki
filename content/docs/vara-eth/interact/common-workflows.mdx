...
const tx = await program.vault.deposit({ value: ethers.parseEther('0.01') });
```

The attached value is credited through the Mirror payable flow and handled by program logic during execution.
