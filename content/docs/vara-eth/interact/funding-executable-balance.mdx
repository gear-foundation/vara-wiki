...
// 1. Approve Router to spend wVARA
...
<Callout type="note" title="Approval Required">
  You must first approve the Router contract (not the Mirror) to spend your wVARA tokens.
</Callout>
...
```typescript
// Via RPC
const balance = await api.executableBalance(programId);
console.log(`Executable balance: ${ethers.formatUnits(balance, 18)} wVARA`);
```
...
For executable (wVARA) balance monitoring, use `@vara-eth/api` (`api.executableBalance(programId)`) or your own RPC-based monitoring.
...
| **Free Balance**       | General-purpose wallet for the program     | Program logic (can withdraw or convert) |
