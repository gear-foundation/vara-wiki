...
| `value`     | Optional wVARA amount to send with the message           |
...
<Callout type="tip" title="When to Use Each Path">
  **Ethereum-side**: On-chain value transfer, Solidity composability, L1 receipts **Vara.eth-side**: Instant feedback
  for users (Web2-like UX), no immediate L1 settlement needed
</Callout>
...
For instant feedback, use pre-confirmations via the Vara.eth RPC. The [sails-js](/docs/developing/sails-js) SDK handles this automatically:

```typescript
const tx = await program.counter.increment();
// Pre-confirmed result — sub-second
const preConfirmed = tx.preConfirmedResult;
// Final result — after L1 settlement
...
Messages can carry **two types of value**:

### wVARA Value

wVARA (ERC-20 token) is credited to the program's Free Balance:

```typescript
// Approve Router to spend wVARA first
await wvara.approve(routerAddress, amount);

// Send message with wVARA value
...
<Callout type="note" title="Value Flow">
**wVARA**: User's wVARA → Router → Program's Free Balance  
**Ether**: User's ETH → Mirror → Program's Owned Balance
