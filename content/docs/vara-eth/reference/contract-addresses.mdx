...
The Mirror address is deterministic and emitted in the `ProgramCreated` event:
...
router.on("ProgramCreated", (actorId, mirrorAddress) => {
  console.log(`Program ${actorId} -> Mirror at ${mirrorAddress}`);
});
```
...
interface IRouter {
    ...
    // Events
    event ProgramCreated(bytes32 actorId, address mirror);
    event CodeGotValidated(bytes32 codeId, bool valid);
    event BatchCommitted(bytes32 batchHash);
