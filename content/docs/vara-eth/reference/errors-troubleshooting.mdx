...
Sails build tooling resolves the target automatically and emits artifacts in `target/wasm32-gear/release/`.
...
4. **Validators not processing** — No executor network is reachable. The campaign validators are retired; run [your own node](/docs/vara-eth/setup/local-node).
...
```bash
cargo sails client-js program.idl ./src/generated/lib.ts
```
...
**Fix**: Pre-confirmations require a reachable executor network. The Gear-operated validators from the launch campaigns are retired, so point the Vara.eth provider at [your own node](/docs/vara-eth/setup/local-node) — or fall back to waiting for L1 finality, which needs no executor at all.
