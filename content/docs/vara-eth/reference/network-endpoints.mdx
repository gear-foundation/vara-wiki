...
<Callout type="note" title="Network Availability">
  Public Vara.eth endpoints listed below (RPC, Beacon, validator network) are currently offline between campaign phases and are kept for reference. To experiment today, [run your own node](/docs/vara-eth/setup/local-node).
</Callout>
