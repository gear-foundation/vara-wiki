...
<Callout type="note" title="Network Availability">
  Public Vara.eth RPC endpoints (including the gear-tech.io Hoodi ones shown below) are currently offline between campaign phases. The full local stack works regardless — it depends on nothing external.
</Callout>
