...
Anyone can run their own Vara.eth node and experiment. There are two modes:

1. **Node-only mode** — connect your local node to an existing Ethereum Router (for example, Hoodi). Requires a working Ethereum RPC endpoint.
2. **Full local stack (recommended)** — run local Ethereum + deploy Router yourself + connect node to it. Fully self-contained.

<Callout type="note" title="Endpoint Availability">
  The Ethereum RPC endpoints Gear operated for the launch campaigns are no longer served, so `--ethereum-rpc` below is a placeholder. Node-only mode still works against the deployed Hoodi and Mainnet Routers — point it at any Ethereum RPC provider for that chain. The full local stack depends on nothing external at all.
</Callout>
...
Use this when you already have a Router address (for example on Hoodi). The Routers from the campaigns are still deployed — see [Contract Addresses](/docs/vara-eth/reference/contract-addresses).

```bash
ethexe run \
  --ethereum-rpc "<YOUR_HOODI_RPC_WS>" \
  --ethereum-router "0xE549b0AfEdA978271FF7E712232B9F7f39A0b060" \
  --rpc-port 9944
```

`--ethereum-rpc` takes a plain Ethereum node WebSocket URL, so any Hoodi (or Mainnet) provider works.

<Callout type="tip" title="When to Use Node-Only">
  Node-only mode is enough to inspect behavior, test RPC flows, and run integration checks against an existing network
  Router — provided you have a working Ethereum RPC for that network.
</Callout>
...
## Switch Back to Hoodi

To point back at the deployed Hoodi Router:

1. Stop your local node
2. Run `ethexe tx` commands with your Hoodi RPC and the Router address (or put them into `.ethexe.toml`):
   ```bash
