...
graph TB
    subgraph eth[Ethereum L1]
        ...
        User -->|sendMessage| Mirror
        Mirror -.->|events| Router
        Router -->|state updates| Mirror
    end

    subgraph vara[Vara.eth Network]
        Exec1[Executor]
        Exec2[Executor]
        Exec3[Executor]
        Program[WASM Program]

        Exec1 <-->|execute| Program
        Exec2 <-->|execute| Program
        Exec3 <-->|execute| Program
    end

    Router -.->|observe events| Exec1
    Exec1 -->|commit batch| Router

    style eth fill:#e1f5ff
    style vara fill:#e8f5e9
```
