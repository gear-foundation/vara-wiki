...
browser_title: Developing on Vara Network
custom_title: Developing on Vara Network
---
...
**Modern development doesn't happen without AI agents anymore.** Whether you're shipping a real product or building your first dApp, you describe what you need and the agent — Claude Code, Codex, Cursor — produces the code. Vara ships a complete, open-source skill pack purpose-built for this loop, ready to use here and now.

[Vara Skills](/docs/vara-network/api/tooling/vara-skills) gives the agent the exact APIs, real imports, and verified patterns for every step — spec, Sails contract, IDL, frontend, tests, and deployment. No hallucinated symbols, no broken builds.

This is the recommended path for new projects today. And the agent loop reaches beyond building: the [Trading Skill](/docs/vara-network/api/tooling/trading-skill) lets an agent trade VARA and other spot pairs on MEXC, Gate.io, and Coinbase, with local-only keys and dry-run-first safety.

## Messaging Concept

Understanding the message communication model and its benefits is of utmost importance for the safety and success of developers working with Vara Network. This innovative model is built upon the principles of the [Actor Model](/docs/vara-network/about/technology/actor-model), where all participants, including programs and users, are considered "actors." These actors interact seamlessly by exchanging messages.

Explore the comprehensive resources available in [this Wiki](/docs/developing/build/introduction) to delve deeper into the development process and leverage the full potential of the message communication model. This valuable reference provides clear insights and guidance on implementing smart contracts with efficiency and precision.
...
{/*Visit [this article](/docs/developing/build/rent-system/) to explore the technical aspects of the rent program system.*/}

## How to Start Developing on Vara Network?

1. Learn program development using the [developer documentation](/docs/developing/build/introduction).
2. Explore and try a batch of ready-made programs on [GitHub](https://github.com/gear-foundation).
3. For keeping up with all the latest Gear & Vara technical updates and releases, join this Telegram channel - https://t.me/GearVaraDev.