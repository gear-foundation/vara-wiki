...
This is the recommended path for new projects today. And the agent loop reaches beyond building: the [Trading Skill](/docs/vara-network/api/tooling/trading-skill) lets an agent trade VARA and other spot pairs on Gate.io and Coinbase, with local-only keys and dry-run-first safety.
