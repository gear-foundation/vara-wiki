---
title: Gasless and Signless
custom_title: Gasless and Signless Transactions
---
...
This innovative feature empowers actors to issue [vouchers](/docs/developing/api/vouchers) that enable specific users to send messages to designated programs without incurring gas fees. This paradigm-shifting approach fosters a sponsorship-like environment for users, transforming the way interactions take place in the Web3 ecosystem.

An example of using vouchers is shown in the [Battleship](/docs/vara-network/examples/Gaming/Battleship) game. Users without tokens in their balance can interact with a program by sending messages using a voucher, freeing them from the need to sign every transaction when interacting with the blockchain.
...
Learn how to create and use vouchers in [this article](/docs/developing/api/vouchers).
