...
Anyone can create a Staking program and run it on the Gear Network. The source code, developed using the [Sails](/docs/developing/build/sails) framework, is available on [GitHub](https://github.com/gear-foundation/dapps/tree/master/contracts/staking).

The staking program interacts with the [Vara Fungible Token (VFT)](/docs/vara-network/examples/Standards/vft) standard to handle token transfers seamlessly, enabling staking deposits, reward distributions, and withdrawals through secure and efficient transfer operations. These transfers are executed via asynchronous calls to the VFT contract, ensuring accurate state synchronization and error handling during token movements.
...
For more details about testing programs written on Vara, refer to this article: [Program testing](/docs/developing/build/testing).
