---
title: ZK-Battleship Game
---

## Introduction

[The previous paper](/docs/vara-network/examples/Gaming/Battleship)  explored the implementation of a Battleship game, where all data, including the positions of players' ships, were openly stored on a blockchain. While this approach demonstrated the potential of decentralized gaming, it had a significant drawback: the ship positions were publicly accessible, undermining the strategic nature of the game.
...
<Callout type="note" title="Fair Play & Privacy">
    This process ensures game integrity by preventing players from altering ship placements or falsifying shot outcomes.

    By moving the logic on-chain, the game maintains a **trustless environment** where cheating is technically impossible.
</Callout>
...
In the context of this program, performing BLS computations directly on-chain would significantly increase gas costs and processing time, potentially requiring over 30 blocks on the Vara network to complete. To address this, the program utilizes the [BLS12-381 Built-in Actor](/docs/developing/build/builtinactors/bia-bls), a specialized module integrated into the Vara runtime.
...
<Callout type="idea" title="Cryptographic Optimization">
    This implementation leverages **off-chain computation** via the **BLS12-381 Built-in Actor**.

    By offloading complex cryptographic operations to the runtime, the program significantly **optimizes performance** and **reduces gas costs**.
</Callout>
...
![zk general scheme](/images/docs/examples/zk-general-scheme.png)
