---
title: Web3 Warriors
custom_title: Web3 Warriors Battle
---
...
```
<Callout type="idea" title="Automated Move Logic">
    The `Start Battle` function utilizes **Delayed Messages** and **Gas Reservation** to ensure game continuity:

    * **Mechanism:** After pair formation, `send_delayed_message_make_move_from_reservation` is triggered.
    * **Purpose:** It uses the participant's pre-allocated gas to force an automatic move if they fail to act within the time limit.
    * **Benefit:** This prevents the game from getting stuck, ensuring a smooth experience for all players.
</Callout>
