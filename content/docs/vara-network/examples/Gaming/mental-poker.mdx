...
All verification logic relies on pairing-friendly cryptography using [BLS12-381](/docs/developing/build/builtinactors/bia-bls) and Bandersnatch, providing compatibility with modern proof systems such as Groth16 and PLONK.

**Built-in Contracts for Verification:**  
On-chain zk-SNARK proof verification (Groth16/PLONK) is always performed by a dedicated zk-verification contract, never inside the game contract itself. Vara exposes **[built-in actors](/docs/developing/build/builtinactors/)** that provide highly optimized cryptographic operations — multi-scalar multiplication, pairing checks, and related primitives. This approach accelerates verification, minimizes gas usage, and isolates security-sensitive code.
...
```rust
@query
PlayerCards(player_id: actor) -> Option<[EncryptedCard; 2]>;
```
...
- **UI:** [zk-poker frontend](https://github.com/gear-foundation/dapps/tree/master/frontend/apps/zk-poker)   
- **Built-in Actors (BLS):** [BLS12-381 Built-in Actor](/docs/developing/build/builtinactors/bia-bls)
