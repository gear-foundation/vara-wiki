...
service CarRacesService {
    events {
        RoundInfo(RoundInfo),
        GameFinished { player: actor },
        Killed { inheritor: actor },
    }
    functions {
        // Admin actions
        AddAdmin(admin: actor);
        RemoveAdmin(admin: actor);
        UpdateConfig(config: ServicesConfig);
        AllowMessages(messages_allowed: bool);
        Kill(inheritor: actor);
        AddStrategyIds(car_ids: [actor]);
        RemoveGameInstance(account: actor);
        RemoveInstances(player_ids: Option<[actor]>);

        // Player (game-related) actions
        StartGame(session_for_account: Option<actor>);
        PlayerMove(strategy_move: StrategyAction, session_for_account: Option<actor>);
        ...
        AllGames() -> [(actor, Game)];
        @query
        ConfigState() -> ServicesConfig;
        @query
        DnsInfo() -> Option<(actor, String)>;
        @query
        Game(account_id: actor) -> Option<Game>;
        @query
