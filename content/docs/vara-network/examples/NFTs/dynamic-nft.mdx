...
This example demonstrates Gear Protocol's unique features enabling the new user experience for totally on-chain, truly decentralized applications that do not require centralized components. [Delayed messages](/docs/developing/build/gstd/delayed-messages) allows the contract to wake itself after a specified period of time.
...
<Callout type="note" title="Scope of this Guide">
    This is an extended version of the standard [vNFT](/docs/vara-network/examples/Standards/vnft).
...
pub fn update_metadata(
    ...
    self.emit_event(Event::MetadataUpdated { token_id, current_media_index })
        .expect("Notification Error");
}
```
