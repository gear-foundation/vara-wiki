...
![img alt](/images/docs/examples/nft-marketplace.png)
...
A program can be deployed to the network using [idea.gear-tech.io](https://idea.gear-tech.io/). In the network selector, choose `Vara Network Mainnet` or `Development` (in this case, the application will connect to a locally running Vara node).
...
```sh
REACT_APP_NODE_ADDRESS=wss://rpc.vara.network
...
- `REACT_APP_NODE_ADDRESS` is Vara Network address (for mainnet it is - `wss://rpc.vara.network`)
...
```rust title="nft-marketplace/app/src/lib.rs"
    ...
    pub async fn add_market_data(&mut self, nft_contract_id: ContractId, ft_contract_id: Option<ContractId>, token_id: TokenId, price: Option<Price>) {
        ...
        self.emit_event(MarketEvent::MarketDataAdded { nft_contract_id, token_id, price })
...
```rust title="nft-marketplace/app/src/lib.rs"
    ...
    pub async fn buy_item(&mut self, nft_contract_id: ContractId, token_id: TokenId) {
        ...
        self.emit_event(MarketEvent::ItemSold { owner: msg_src, nft_contract_id, token_id })
            .expect("Notification Error");
    }
```
