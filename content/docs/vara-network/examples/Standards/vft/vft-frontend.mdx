---
title: VFT Frontend Guide
custom_title: VFT Extended DApp Frontend Integration Guide
---

![VFT frontend app example](/images/docs/examples/Standards/vft-app.png)
...
[See VFT Standard description](/docs/vara-network/examples/Standards/vft)
...
- Access to **Vara Network** (a node endpoint and some VARA to pay for transactions)
...
<Callout type="warn" title="Prerequisites">
    Before starting the frontend development, ensure your contract is **uploaded to the Vara Network** and you have its **Program ID** ready.

    Interaction with the network is impossible without a deployed contract.
</Callout>
...
Use the official Vara React app [template](/docs/developing/api/tooling/create-gear-app).
...
<Callout type="idea" title="Template Features">
    The provided template is production-ready and includes:

    * **Wallet Connection:** Pre-configured interaction with Vara-compatible wallets.
    * **Node Provider:** Pre-set connection to the Vara Network.
    * **Alert System:** Built-in notifications for user feedback.
    * **Contract Ready:** Fully prepared for smart contract integration.
</Callout>
...
Make sure your VFT contract is already deployed on Vara Mainnet (via [Gear IDEA](https://idea.gear-tech.io)) and you have its address (ProgramId).

### Generate Client Library

Follow the client generation instructions: [Client Generation Docs](/docs/developing/sails-js/client-generation)
...
<Callout type="idea" title="Type-Safe API Access">
    The generated file (IDL) provides a full description of your contract's API.

    Use it to enable **type-safety** on the frontend, ensuring that your calls match the contract's expected arguments and return types.
</Callout>
...
<Callout type="info" title="Additional Resources">
    For detailed documentation on **Sails React Hooks** and up-to-date API examples, check out these links:

    * [React Sails Hooks Documentation](/docs/developing/sails-js/react-hooks)
...
<Callout type="note" title="API Reference">
    For a full list of query methods and detailed usage, see the [React Hooks API Reference](/docs/developing/sails-js/react-hooks).
</Callout>
...
<Callout type="idea" title="When to use watch?">
    The `watch` parameter determines how frequently your data updates:

    * **Use `watch: true`:** For dynamic data like **balances** or values affected by frequent user actions and contract events.
    * **Skip `watch`:** For static data such as **token names, symbols, or decimals** to improve app performance.
</Callout>
...
<Callout type="info" title="Full Implementation Example">
    For a complete, production-ready example of the hooks setup, refer to the source code: [hooks/api.ts on GitHub](https://github.com)
</Callout>
...
<Callout type="warn" title="Security First">
    **Never hard-code private keys** or sensitive data in the frontend.
    ...
    Always use environment variables (`.env`) for node endpoints and contract addresses to keep your application secure and configurable.
</Callout>
