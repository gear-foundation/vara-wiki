---
title: VMT (ERC-1155)
custom_title: Vara Multi-Token (VMT) Standard and Extended Implementation
---

<Callout type="note" title="ERC-1155 Analogue">
    The **Vara Multi-Token (VMT)** standard is the equivalent of Ethereum's **ERC-1155** on the Vara Network.
</Callout>
...
<Callout type="note" title="VMT Core Service">
    The core VMT service serves as a **foundational framework**. It is designed to be extended with more complex functionality such as **minting, burning, and role management**.

    **Source Code:** [GitHub Repository](https://github.com/gear-foundation/standards/tree/master/vmt-service)
</Callout>
...
service Vmt {
    events {
        Minted { to: actor, ids: [u256], amounts: [u256] },
        Burned { from: actor, ids: [u256], amounts: [u256] },
        Approval { from: actor, to: actor },
        Transfer { from: actor, to: actor, ids: [u256], amounts: [u256] },
    }
    functions {
        Burn(from: actor, id: u256, amount: u256);
        BurnBatch(from: actor, ids: [u256], amounts: [u256]);
        GrantAdminRole(to: actor);
        GrantBurnerRole(to: actor);
        GrantMinterRole(to: actor);
        Mint(to: actor, id: u256, amount: u256, token_metadata: Option<TokenMetadata>);
        MintBatch(to: actor, ids: [u256], amounts: [u256], token_metadata: [Option<TokenMetadata>]);
        RevokeAdminRole(from: actor);
        RevokeBurnerRole(from: actor);
        RevokeMinterRole(from: actor);
        Approve(to: actor) -> bool;
        BatchTransferFrom(from: actor, to: actor, ids: [u256], amounts: [u256]);
        TransferFrom(from: actor, to: actor, id: u256, amount: u256);
        @query
        Admins() -> [actor];
        @query
        Burners() -> [actor];
        ...
        Minters() -> [actor];
        @query
        BalanceOf(account: actor, id: u256) -> u256;
        @query
        BalanceOfBatch(accounts: [actor], ids: [u256]) -> [u256];
        @query
        Decimals() -> u8;
        @query
        IsApproved(account: actor, operator: actor) -> bool;
        @query
        Name() -> String;
        @query
        Symbol() -> String;
        @query
        TotalSupply() -> [(u256, u256)];
    }
    types {
...
<Callout type="note" title="Extended VMT Implementation">
    The **Extended VMT** service demonstrates how to build upon the core logic to include advanced features:

    * **Minting & Burning:** Logic for creating and destroying tokens.
    * **URI Management:** Handling metadata for multi-tokens.

    **Source Code:** [GitHub Repository](https://github.com/gear-foundation/standards/tree/master/extended-vmt)
</Callout>
