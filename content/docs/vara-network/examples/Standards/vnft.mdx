---
title: VNFT (ERC-721)
custom_title: Vara Non-Fungible Token
---

<Callout type="note" title="ERC-721 Analogue">
    The **Vara Non-Fungible Token (VNFT)** standard is the equivalent of Ethereum's **ERC-721** on the Vara Network.
</Callout>
...
service Vnft {
    events {
        Minted { to: actor, token_metadata: TokenMetadata },
        Burned { from: actor, token_id: u256 },
        Transfer { from: actor, to: actor, token_id: u256 },
        Approval { owner: actor, approved: actor, token_id: u256 },
    }
    functions {
        Burn(from: actor, token_id: u256);
        GrantAdminRole(to: actor);
        GrantBurnerRole(to: actor);
        GrantMinterRole(to: actor);
        Mint(to: actor, token_metadata: TokenMetadata);
        RevokeAdminRole(from: actor);
        RevokeBurnerRole(from: actor);
        RevokeMinterRole(from: actor);
        Approve(approved: actor, token_id: u256);
        Transfer(to: actor, token_id: u256);
        TransferFrom(from: actor, to: actor, token_id: u256);
        @query
        Admins() -> [actor];
        @query
        Burners() -> [actor];
        ...
        TokenId() -> u256;
        @query
        TokenMetadataById(token_id: u256) -> Option<TokenMetadata>;
        @query
        TokensForOwner(owner: actor) -> [(u256, TokenMetadata)];
        @query
        BalanceOf(owner: actor) -> u256;
        @query
        GetApproved(token_id: u256) -> actor;
        @query
        Name() -> String;
        @query
        OwnerOf(token_id: u256) -> actor;
        @query
        Symbol() -> String;
    }
    types {
