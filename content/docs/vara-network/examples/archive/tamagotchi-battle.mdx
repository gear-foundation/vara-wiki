...
![tamagotchi-battle](/images/docs/examples/tamagotchi-battle.png)

Welcome to the exciting world of Tamagotchi Battle! In this fascinating game your faithful companions - Tamagotchi - are ready to fight each other in pursuit of glory and victory. In this game, you will be able to use the tamagotchi that was described in [previous article](/docs/vara-network/examples/archive/tamagotchi).
...
<Callout type="warn" title="Inactivity Penalty">
    The game uses **Gas Reservation** and **Delayed Messaging** to enforce turn limits:

    * **Mechanism:** The `split_into_pairs` function schedules a delayed move check via `msg::send_delayed_from_reservation`.
    * **Consequence:** If a player fails to move within the allotted time, they are automatically **penalized** and take **health damage**.

    This ensures that games cannot be stalled by inactive participants.
</Callout>
