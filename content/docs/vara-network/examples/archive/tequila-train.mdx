...
![Tequila Train Game](/images/docs/examples/tequila-train.png)

Typically, the program's state undergoes alterations during its utilization. An integral feature of the software implementation of this game is the ability to maintain individual timers for each player and to closely monitor time intervals, thus increasing the dynamism and intrigue of the game. Such functionality is achieved through the utilization of [Delayed messages](/docs/developing/build/gstd/delayed-messages), which constitute one among several distinctive attributes of the Gear Protocol.
