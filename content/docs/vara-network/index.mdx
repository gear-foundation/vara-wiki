...
<Callout type="idea" title="Stay Connected">
    Subscribe to the [@GearVaraDev](https://t.me/GearVaraDev) Telegram channel to get the latest releases and updates directly from the development team.
</Callout>
...
- A robust [NPoS](/docs/vara-network/staking) mechanism for network security and decentralization.
- [Governance](/docs/vara-network/governance) by community members - token holders.
- A stable and upgradable network, facilitated by Substrate.
- A well-thought-out [token economy](/docs/vara-network/tokenomics) that ensures the long-term sustainability of the network and benefits for its community.

<Callout type="warn" title="Vara Network: BETA Phase">
    Vara is currently in the **BETA phase**. While the network is live, it is not guaranteed to be entirely bug-free.

    However, thanks to **fork-less upgrades**, the network will evolve continuously while ensuring all user data is preserved.
</Callout>

## Built for the Agentic Era

**Modern software development runs on AI agents.** Whether you're shipping a production dApp or prototyping an idea, you describe what you need and the agent — Claude Code, Codex, Cursor — writes the code. Vara is built for this loop, with a complete, open-source skill pack you can use right now.

**[Vara Skills](/docs/vara-network/api/tooling/vara-skills)** gives the agent the exact APIs, real imports, and verified patterns for every step — spec, Sails contract, IDL, frontend, tests, and deployment. No hallucinated symbols, no broken builds. You describe the dApp; the agent ships it.

Agents don't stop at building. The **[Trading Skill](/docs/vara-network/api/tooling/trading-skill)** lets an agent trade VARA and other spot pairs on centralized exchanges — MEXC, Gate.io, and Coinbase — with guided onboarding, local-only keys, risk limits, and dry-run-first safety.

This is available **here and now**, not on a roadmap. For Vara.eth (Ethereum-anchored programs), see [Vara.eth Agent Skills](/docs/vara-eth/setup/agent-skills).
...
1. Learn program development using the [developer documentation](/docs/developing/build).
