...
custom_title: Backup and Restore the Node
---
...
The node can connect to different chains. The chain can be selected using the `--chain` argument. When connected to Vara Mainnet (`--chain=vara`), the chain subdirectory name is `vara_network`, resulting in the `gear/chains/vara_network` path.
...
```shell
hexdump -e '1/1 "%02x"' /root/.local/share/gear/chains/vara_network/network/secret_ed25519
...
To move the node to a new server, backup and restore the following (provided paths are for a Vara Mainnet node):

- The network private key of the node:

    - Linux: `$HOME/.local/share/gear/chains/vara_network/network/secret_ed25519`
    - macOS: `$HOME/Library/Application Support/gear/chains/vara_network/network/secret_ed25519`
    - Windows: `%USERPROFILE%\AppData\Local\gear.exe\chains\vara_network\network\secret_ed25519`

- (optional) The database:

    - Linux: `$HOME/.local/share/gear/chains/vara_network/db/full`
    - macOS: `$HOME/Library/Application Support/gear/chains/vara_network/db/full`
    - Windows: `%USERPROFILE%\AppData\Local\gear.exe\chains\vara_network\db\full`
...
<Callout type="warn" title="Stop Before Backup">
    **Stop the node** before backing up the database to prevent data corruption.
