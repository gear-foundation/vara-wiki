...
- **Error:** `IO error: While lock file /root/.local/share/gear/chains/vara_network/db/full/LOCK: Resource temporarily unavailable`
...
- **Solution:** The root of this problem is the lack of the disk free space. You may check the free space using the following command:
    ...
    ```shell
    du -h $HOME/.local/share/gear/chains/vara_network/db/full
    ```

    Please refer to the [System Requirements](/docs/vara-network/node#system-requirements) to see the minimum disk space required.
...
- **Solution:** [Update](/docs/vara-network/node/node-as-service#updating-the-node-to-a-new-version) the node binary to the latest version.
