...
```bash
sudo apt install -y binaryen build-essential clang cmake curl git jq libssl-dev pkg-config
```
...
<Callout type="note" title="Fix 'Command Not Found' Error">
    ...
    **Run this command to update your PATH:**
    ...
    Alternatively, simply restart your terminal (or VS Code) to apply the changes.
</Callout>
...
<Callout type="warn" title="Required Build Target">
...
In `Cargo.toml`, the essential libraries required for building your first project have been included, for example:
    ...
    [dev-dependencies]
    sails-rs = { version = "1.0.0", features = ["gtest", "gclient"] }
...
## Deploy your program to Vara Mainnet

Gear provides an application for developers (Gear Idea) that implements all of the possibilities of interaction with programs on Vara Network, available at [idea.gear-tech.io](https://idea.gear-tech.io).

<Callout type="info" title="Build directly on Mainnet">
    Building a dApp used to mean an extra step - spin up on a testnet, then move everything to mainnet. That step is gone. Gear Idea hands every developer **100 free VARA per wallet, once**, so you can deploy, test and cover your first transactions directly on mainnet. Real network, real conditions, nothing to pay to start.
</Callout>
...
2. Once downloaded, click <kbd>+</kbd> button to create a new account:
    ...
    ![Add account](/images/assets/getting-started/polkadot-add-acc.png)

3. Make sure you save your 12-word mnemonic seed securely.

    ![Create account](/images/assets/getting-started/polkadot-add-acc-2.png)
...
4. Select the network that will be used for this account - choose "Allow to use on any chain". Provide any name to this account and password and click "Add the account with the generated seed" to complete account registration.
    ...
    ![Finalizing account](/images/assets/getting-started/polkadot-add-acc-3.png)
...
5. Go to **[idea.gear-tech.io](https://idea.gear-tech.io)**. You will be prompted to grant access to your account for the application, click "Yes, allow this application access".
    ...
    ![Allow access](/images/assets/getting-started/polkadot-access.png)

6. Make sure you are connected to the `Vara Network Mainnet`. The network name is at the bottom left corner of the page.

    ![Network name](/images/assets/getting-started/idea-network.png)

7. You may switch the network by clicking on the network name.

    ![Switch network](/images/assets/getting-started/switch-network.png)

8.    Click the `Connect` button on the top-right to select an account that will be connected to Vara.

    ![Connect account](/images/assets/getting-started/connect-account.png)

9. In accordance with the Actor model, programs are uploaded to a network via messages. Vara node charges a gas fee during message processing. Your account balance needs to have enough funds to upload a program to Vara Mainnet. If your wallet is new, claim the one-time developer grant of **100 VARA** - click the following button:

    ![Get balance](/images/assets/getting-started/get-balance.png)

    A notification about successful balance replenishment will appear after passing captcha at the bottom of the window. You can also see the current account balance next to the account name in the upper right corner. The grant is available once per wallet and is enough to deploy a program and cover your first transactions.

    ![Transfer balance](/images/assets/getting-started/got-balance.png)
...
1. When your account balance is sufficient, click the <kbd>Upload program</kbd>  button to open a popup window for uploading your new program.
    ...
    ![Upload program button](/images/assets/getting-started/upload.png)

2. In the popup, click the <kbd>Select file</kbd> button and navigate to the `.opt.wasm` file we have pointed to above

    ![Upload new program](/images/assets/getting-started/upload-new-program.png)
...
3. After uploading the `.opt.wasm` file, you need to upload the `IDL` file. Click the <kbd>Select file</kbd> button and navigate to the `.idl` file referenced earlier.
    ...
    ![Upload idl button](/images/assets/getting-started/add_idl.png)
...
4. Specify the program name, click the <kbd>Calculate Gas</kbd> button to set the gas limit automatically, and then click the <kbd>Submit</kbd> button.
    ...
    ![Upload program form](/images/assets/getting-started/interface.png)
...
5. Sign the `gear.uploadProgram` transaction to Vara. It is recommended to set the checkbox `Remember my password for the next 15 minutes` for your convenience.
    ...
    ![Sign transaction](/images/assets/getting-started/sign-transaction.png)

6. Once your program is uploaded, go to the `Programs` section, find your program, and select it.

    ![Recently uploaded programs](/images/assets/getting-started/recent.png)

7. Click on your program to see more information, such as `Metadata/Sails`, `Messages`, `Events`, and the `Vouchers` pane.

    ![Main page program](/images/assets/getting-started/main-page-program.png)
...
2. Click the <kbd>Calculate Gas</kbd> button to set the gas limit automatically, then click the <kbd>Send Message</kbd> button.
    ...
    ![Send form](/images/assets/getting-started/send-request.png)
...
4. After your message has been successfully processed, you will see corresponding notifications:
    ...
    ![Log](/images/assets/getting-started/message-log.png)
...
6. In the message pane, you can view the corresponding response.
    ...
    ![Log](/images/assets/getting-started/messages.png)
...
7. Click on the message response to see more information. The expected "**Hello from VaraApp!**" response can be seen in the `Payload`.
    ...
    ![Log](/images/assets/getting-started/reply.png)

---

## Further reading

For more info about writing programs for Vara and the specifics behind the program implementation, refer to [this article](/docs/developing/build/introduction).
