...
custom_title: Vara Validator FAQs
---
...
- Tokenomics: [https://wiki.vara.network/docs/vara-network/tokenomics/](/docs/vara-network/tokenomics)
...
<Callout type="info" title="Automatic Chain Configuration">
    These nodes are already included in the [Vara chain spec](https://raw.githubusercontent.com/gear-tech/gear/master/node/res/vara.json).

    This configuration is applied **automatically** when you start your node with the `--chain vara` flag.
</Callout>
...
Please follow this guide to get started: [Run Vara Validator](/docs/vara-network/staking/validate/).
...
You can find related information on the Vara wiki page: [Run Vara Validator > Hardware requirements](/docs/vara-network/staking/validate/#hardware-requirements)
...
For instructions on how to use it, check this [article](/docs/vara-network/staking/validate/#-steps-to-use-a-snapshot).
...
![liveness slashing penalty](/images/docs/staking/slashing-penalty-liveness.png)
...
![equivocation slashing penalty](/images/docs/staking/slashing-penalty-equivocation.png)
...
<Callout type="idea" title="Enable Telemetry">
    To see your node on the [telemetry dashboard](https://telemetry.rs), add the following parameter to your startup command:
...
<Callout type="idea" title="Prometheus Metrics">
    To enable monitoring, specify the prometheus port (e.g., `--prometheus-port 9615`).

    Once started, node metrics will be available locally at: `http://127.0.0.1:9615/metrics`
</Callout>
...
![querying chainstate](/images/docs/staking/querying-chainstate.png)
...
<Callout type="error" title="Critical: Avoid Double Signing">
    **Stop the old node before launching the new one.**

    Running multiple nodes with the same session keys will lead to **equivocation slashes** (direct loss of your staked tokens).
</Callout>
