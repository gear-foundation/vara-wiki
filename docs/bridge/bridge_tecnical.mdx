...
# Vara ↔ Ethereum Bridge Tecnical Overview
...
[📄 Security Audit Report (PDF)](link-to-pdf)
:::

:::important
To explore the code navigate to:  
[🔗 Bridge Implementation Repository](link-to-repo)
:::
...
1. A user wants to move VARA to Ethereum. wVara as erc20 analogue: VFT
2. They lock 100 VARA via the VFT Manager (Gear program).
...
6. The root is accepted, and the user can now submit their message (with Merkle proof). Also on relayers.
...
Integration can happen through the governance process (DAO), allowing the community to review, approve, and add new logic to the bridge registry
