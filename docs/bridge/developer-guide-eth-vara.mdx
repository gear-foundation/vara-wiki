...
At a high level, the cross-chain message flow from **Ethereum → Vara** is as follows:
...
4. **The bridge program on Vara verifies the proof.**  
   If the proof is valid, it calls the custom program/contract on Vara, which can process the “ping” (emit a “pong”, trigger logic, etc.).
5. **The Vara program receives the cross-chain message and acts.**  
   The cycle is complete, with cryptographic auditability on both chains.

## Implementation Scope

No implementation of transport or synchronization logic between Ethereum and Vara is required at the application level.  
All low-level mechanics — including slot synchronization, beacon chain tracking, and finality validation — are handled by the protocol.
...
**Summary:**  
The only application-side focus is on emitting and listening for relevant events, and on writing application logic.  
All cross-chain transport and slot-level synchronization are provided by the bridge infrastructure.
...
2. **Relayer Service (Off-chain)**
    - Monitors specific events on Ethereum (such as `PingFromEthereum`).
...
On Ethereum, a simple contract is deployed that exposes a `ping()` function and emits a `PingFromEthereum` event:
...
#### Relayer Side Implementation Details
...
3. **Calculate the slot for the event:**  
   Slot is calculated based on the **block timestamp** and Hoodi genesis.
    ...
    async function getSlotForEvent(event, provider) {
        ...
        const GENESIS = 1695902400; // Hoodi genesis timestamp
...
On the Vara side, the application is a Rust smart contract (Gear program) that implements a service interface to receive cross-chain messages from Ethereum.

[Source code on GitHub](https://github.com/gear-foundation/eth-vara-cross-ping/tree/master/ping-receiver)

**How it works:**
- The relayer, via HistoricalProxy, delivers messages by calling the `send_message` function (or route) on the program.
...
- The service interface (route) must accept these parameters.
...
- Verification can be performed using [Gear Idea](https://idea.gear-tech.io/) or any Vara explorer by checking the program’s messages and emitted events.