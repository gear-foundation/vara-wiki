...
- The destination contract address (on Ethereum) can be provided at initialization, hardcoded in the program logic, or even passed alongside the message itself.
- The relayer  obtains the destination address from the event emitted by the Vara contract.
- In this example, the contracts on both sides are pre-linked, but this is only one possible scenario—such coupling is entirely defined by the application logic. At the protocol level, nothing changes; the protocol remains agnostic to how the destination is set or discovered.
...
**How it works:**  
The MessageQueue contract calls `processVaraMessage(sender, payload)` on the destination contract whenever a valid message arrives.  
The payload can be handled in any way required by the contract logic (in this example, a `PongEmitted` event is simply emitted).

The sender is set by the original Vara program, and the payload can contain any bytes sent from Vara.
