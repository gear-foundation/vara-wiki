...
- **Solution:** You seem to be running several Vara node instances. Note that only one node instance is allowed to run. You likely have configured the node as a service and then ran the second instance from the command line. You should either stop the service or don't run the Vara node from the command line.
...
- **Solution:** The `gear-node.service` configuration file seems to be misconfigured. Some versions of SystemD do not accept the backslash character (`\`) as a line break. Therefore, it is better to write each of the config entry on one line.
    ...
    Refer to /node/node-as-service for properly configuring the node as a service.
...
- **Solution:** The root of this problem is the lack of the disk free space. You may check the free space using the following command:
    ...
    Please refer to the [System Requirements](/docs/node/node.mdx#system-requirements) to see the minimum disk space required.
...
- **Solution:** [Update](/docs/node/node-as-service#updating-the-node-to-a-new-version) the node binary to the latest version.
